/*
 *  N-Reinas. Trabajo para final de la materia Introducci√≥n a la Computaci√≥n
 *  Evolutiva. Permite correr un algoritmo gen√©tico para resolver el problema
 *  la conocido con el nombre de N-Reinas.
 *  Copyright (C) 2010 Mart√≠n I. Pacheco
 *
 ...
 *
 *  Correo electr√≥nico: mpacheco@alumnos.exa.unicen.edu.ar
 */
...
public class Cruce extends BaseGeneticOperator{
    ...
    public void operate(final Population poblacion, final List cromosomas_candidatos){
            int tamaÒo = Math.min(getConfiguration().getPopulationSize(), poblacion.size());
            int num_cromosomas = (int) (tamaÒo * porcentaje_cruce) / 2;
            RandomGenerator generador = getConfiguration().getRandomGenerator();
            for (int i = 0; i < num_cromosomas; i++) {
                IChromosome primer_conflicto = (IChromosome) poblacion.getChromosome(generador.nextInt(tamaÒo)).clone();
                IChromosome segundo_conflicto = (IChromosome) poblacion.getChromosome(generador.nextInt(tamaÒo)).clone();
   ...
   protected Gene[] operate(final Gene[] g1, final Gene[] g2){
        ...
        while (i < g2_ultimo.length && j < tamaÒo){
        ...
        if (cantElementos(gen_hijo) < gen_hijo.length){
            ...
            while (k < g1_primero.length && l < tamaÒo){
    ...
    protected boolean contieneGen(final Gene[] gen,final int tamaÒo_gen,final  Gene gen_x) {
        for (int i = 0; i < tamaÒo_gen; i++) {
